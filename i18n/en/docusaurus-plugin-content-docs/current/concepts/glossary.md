...
| 术语 | 含义 | 实现落点 |
|------|------|----------|
| **ZTS** | Unity 下 C# 与 JavaScript（QuickJS）互操作框架；Editor 用 Mono，Player 用 Il2Cpp | `Packages/com.code-philosophy.zts` |
| **JS 可见语义** | 脚本侧可观察的行为契约；Mono 与 Il2Cpp **必须一致** | `spec/**` |
| **双运行时** | `ZTS.Mono`（Editor）+ `ZTS.Il2Cpp`（Player）；门面 `TsAppDomain` | [00-OVERVIEW.md](/docs/spec/00-OVERVIEW/) |
| **libil2cpp/zts** | Player 侧 native 实现根目录 | `build-win64/.../libil2cpp/zts` |
| **GetFunction** | C#→JS：`TsAppDomain.GetFunction<T>` 按 ES 模块与导出名绑定 Delegate | [01-HOST-API.md](/docs/spec/01-HOST-API/) |
...
| **typeArg** | `zts` API 中描述 C# 类型的实参：类型对象 / `zts.types.*` / mscorlib 字符串 | `spec/05-LIB.md`§3 |
...
| **属性 miss** | 未注册成员 → **`throw Error`**（`zts: member not found: …`；禁止反射 fallback，禁止静默 `undefined`） | [metatable/02-INDEX.md](/docs/spec/metatable/02-INDEX/) |
| **只写属性读 miss** | setter 存在但 getter 不存在 → `zts: property has no getter: …` | 同上 |
...
| **`[TsMarshalAs]`** | 参数 / 返回值 / 字段 / 属性的 Marshal 标注 | `spec/marshal/02-MARSHAL-AS.md` |
| **`[TsAlias]`** | 为方法追加最终 JS 名；可与默认名/其它别名重复，按名分组进 overload | `spec/04-METHOD-OVERLOAD.md` §5 |
| **Delegate 桥** | C#→JS 调用路径：`GetFunction` 绑定导出函数为 closed delegate 后 `Invoke` | `spec/marshal/09-FUNCTION.md` |
| **ES module** | 脚本模块单位；`GetFunction` 的 `jsModule` 为 **canonical specifier**（**不含** `.js` / `.ts`）；`csharp:` 为 ZTS 保留的类型模块 scheme | [01-HOST-API.md](/docs/spec/01-HOST-API/) §1.3；[14-TYPESCRIPT.md](/docs/spec/14-TYPESCRIPT/) |
