...
> **性质：** 选型与阅读导航，**不是** ZenTS 行为规范。  
> **覆盖：** 何时优先 ZenTS、何时暂缓迁移、本目录怎么读。

## 本目录

| 文件 | 内容 |
|------|------|
| [FEATURES.md](/docs/compare/FEATURES/) | 特性与用法：ZenTS vs Puerts vs 自管 QuickJS；与 ZLua 语义同构说明 |
| 本页（SUMMARY） | 选型一句话 + 阅读顺序 + 诚实边界 |

目前 **没有** 公开四方实测数字页；有数据后再补，**不编造基准**。方法论可对照 [ZLua 性能对比](https://doc.zlua.cn/docs/compare/PERFORMANCE/)（那是 Lua 方案，不可直接当 ZenTS 数字）。

## 对比对象（一句话）

| 方案 | 典型定位 | 与 ZenTS 的关系 |
|------|----------|----------------|
| **ZenTS** | JS/TS ↔ C# 完备互操作；Editor Mono + Il2Cpp Player；懒绑定 | 本文档树的「被评估方」 |
...
2. **确认是否愿维护 Il2Cpp 集成：** [为什么选择 ZenTS](/docs/concepts/why-zents/)、[双运行时](/docs/concepts/dual-runtime/)、[构建](/docs/guides/build/)。
...
3. 不贬低其它方案的适用场景；写清 ZenTS 取舍（libil2cpp 侵入、Generate、TS 闸门等）。
4. 与 ZLua 的关系写清：**语义同构、VM 不同**；不是「ZenTS 比 ZLua 快/慢」这类无依据断言。
...
*对比文档随实现演进更新；冲突时 ZenTS 行为以 `spec/**` 为准。*
