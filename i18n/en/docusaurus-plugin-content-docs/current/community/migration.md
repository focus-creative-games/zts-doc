...
从 **Puerts**、Unity JS 集成、其它宿主式 JS 方案迁到 ZTS 时：先对齐 **模块加载与类型导出模型**，再替换调用点。  
...
| **不侵入 ZTS 核心** | 适配为可选 ES module + Editor 导出工具；**不**改 `CSharp` / `csharp:` 语义 |
...
| **成员调用语义对齐** | 重载、Event、`ref`/`out`、Marshal 仍以 ZTS spec 为准；adaptor **只**解决「类型表怎么拿到」 |
| **泛型旧语法** | 如 `puerts.loadGeneric(...)` → 改写为 `zts.make_generic_type` |
| **默认安装全局** | 须显式 `import` adaptor-init；**新项目不要装 adaptor**，直接用 `csharp:` |
| **BlittableCopy / 指针模型** | 用 ZTS exotic + Registry / `[TsMarshalAs]` |
...
| 阶段 | 做法 |
|------|------|
| **过渡** | 复制包内 `ZTS~/adaptors/` 的 `adaptor.js` + 对应 `ExportTypes.cs` → 生成 `*_export_types.js` → `initAdaptor(exportTypes)` |
...
```text
1. 新工程接入 ZTS（Install；Editor 跑通 js-demo / ts-demo 级冒烟）
...
| `puerts.loadGeneric(...)` | `zts.make_generic_type` |
| `JsEnv.Tick` / 帧泵 | 宿主 **`TsFramePump`**（见 Host 规范） |
| `BlittableCopy` | `[TsMarshalAs]` / 默认 Marshal |
| `require('m')` | `import` + `moduleLoader` |
| `obj:Method()`（若从 Lua 思维带来） | `obj.Method()` |
| Eval / JsFunction 回调表 | **`TsAppDomain.GetFunction<T>`** + named export |
...
| 4 | Event / ref / 未知成员行为符合 ZTS guides，而非旧框架习惯 |
