...
`JsAppDomain` 是 ZenTS **唯一推荐的公开宿主入口**：初始化 / 整域重置 QuickJS，以及用 **`GetFunction`** 从 ES 模块按 **named export** 取得 JS callable 对应的 Delegate。宿主面 **不** 暴露 `Shutdown`；清空脚本世界只走 `Reset`。
...
`JsAppDomain` 在 `ZenTS.Common`；Common **不** 引用 Mono/Il2Cpp。首次 `Initialize` / `GetFunction` 时按环境 **反射创建** 后端嵌套类型 `Runtime : IJsRuntime`：

| 环境 | 后端 | 程序集 |
|------|------|--------|
| Editor | `JsMonoAppDomain` | `ZenTS.Mono` |
| Player | `JsIl2CppAppDomain` | `ZenTS.Il2Cpp` |
...
| 真正执行 | 本帧 **EndOfFrame**（`JsFramePump` / `WaitForEndOfFrame`）：排空 pending ref → 关闭 Registry / 模块缓存 → 释放 JS 运行时 → 按 Initialize 路径重建并安装 loader |
...
| Marshal | `Invoke` 时遵循 [Marshal 概览](/docs/concepts/marshal-overview/)；可用 `[JsMarshalAs]` |
...
| `csharp:` | ZenTS **在** 宿主 loader **之前** 拦截合成 CLR 类型模块；**禁止** 把 `csharp:` 传给业务 loader |
...
sequenceDiagram
    ...
    participant TAD as JsAppDomain
    participant BE as 后端 Mono / Il2Cpp
    participant Q as JSRuntime + JSContext

    App->>TAD: Initialize(moduleLoader)
    TAD->>BE: 解析 ZenTS.Mono 或 ZenTS.Il2Cpp
    BE->>Q: 创建运行时、注册 CSharp / zents
    BE->>BE: 安装 moduleLoader、JsFramePump
...
3. **异常边界**：JS `throw` → C# 异常；C# 异常 → JS `Error('zents: …')`（见 [01-HOST-API](/docs/spec/01-HOST-API/) §6）。
