...
> **ZTS 状态：** Alpha；Editor Mono 与 Il2Cpp Player 主路径可用（见 [项目状态](/docs/getting-started/project-status/)）。  
...
| **C#→脚本** | `TsAppDomain.GetFunction<T>(module, export)` | `JsEnv.Eval` / 回调表 / `[JSFunction]` 等 | 自研 eval / 函数句柄 |
...
| **Il2Cpp** | 官方 `zts-runtime` 嵌入 libil2cpp | 有成熟路径 | 自建 |
...
| **ZTS** | 首次访问类型时 **EnsureBinding**；**无** per-type 托管 Wrap 白名单 | Editor 开箱；Player 须 **`ZTS/Generate/All`**（C++ stub，不是 C# Wrap 海） |
...
| 方案 | 静态 | 实例 |
|------|------|------|
| ZTS | `Demo.Add(1, 2)` | `obj.GetX()`（**点号**；无 Lua 冒号） |
| Puerts | `CS.Demo.Add(1, 2)` | `obj.GetX()` |
| 自管 | 视绑定 | 视绑定 |

ZTS：静/实例元数据分离；继承成员在 **Bind 期扁平化**；未知成员 **strict miss** → `throw Error('zts: …')`，**不**静默 `undefined`。
...
| 自管 | 自定 |
| **ZTS** | Bind 期注册；默认最佳匹配；`[TsAlias]` / 显式绑定见 [重载](/docs/guides/overloads/) |
...
| 自管 | 视实现 |
| **ZTS** | 读 / 写未知键均 **throw**（与 ZLua 同构的严格策略） |
...
|------|-----------|------------------------|
| **ZTS** | `TsAppDomain.GetFunction<T>("app", "add")` | 形参隐式 marshal（`Action`/`Func` 等） |
...
| 方案 | 加载 |
|------|------|
| ZTS | `TsAppDomain.Initialize(moduleLoader)` + ES module；`csharp:` 由运行时拦截 |
| Puerts | 自有 loader / require 习惯（视版本） |
| 自管 | 自定 |

Player 读 StreamingAssets（`Js/` 或 `ZTS/`），不是工程旁源目录——见 [构建](/docs/guides/build/)。
...
| **ZTS** | **无** Event 专用对象；`obj.add_OnX(fn)` / `obj.remove_OnX(fn)`；remove 须 **同一 function 引用** |
...
| 主题 | ZTS |
|------|-----|
| C#→JS（`GetFunction`） | byref 默认 **OpaqueValue**；用 `zts.get_opaquevalue` / `set_opaquevalue` |
...
| 与 Puerts BlittableCopy | **不对等**；用 `[TsMarshalAs]` / 默认 Marshal，勿假设指针模型 |
...
| 官方工程 | `ZTS/Init TypeScript Project` → `TsProject/` | 社区 / 自建 | 自建 |
| C# 类型声明 | **`ZTS/Generate Typings`** → `csharp:` modules | 视工具链 | 手写 `.d.ts` |
...
| 自管 | 自定 | 自定 |
| **ZTS** | Mono（Editor）vs Il2Cpp（Player）实现不同、**语义须一致** | Il2Cpp：**`ZTS/Generate/All`**（C++ stub）；**C#→JS 无 codegen** |
...
| 参考方法论 | [ZLua PERFORMANCE](https://doc.zlua.cn/docs/compare/PERFORMANCE/)（Lua 方案，**不可**直接引用为 ZTS 结果） |

完备互操作与懒绑定的取舍，见 [为什么选择 ZTS](/docs/concepts/why-zts/)。
...
| □ ref / struct | 按 ZTS Opaque / ByVal 改写，勿假设 BlittableCopy |
| □ 泛型 | `loadGeneric` 等 → `zts.make_generic_type` |
...
| 更适合 | 方案 |
|--------|------|
| JS/TS + 完备互操作 + Il2Cpp，愿维护 Install/Generate，要与 ZLua 同构心智 | **ZTS** |
