...
ZTS 目前为 **Alpha**：API 与规范可能随版本迭代，但核心互操作路径（双向调用、Marshal 主路径、TS 工作流）已在 Editor Mono 与 Il2Cpp Player（含 Win64 / Android / iOS 等）上通过内部矩阵验证。平台范围见 [兼容性](/docs/getting-started/compatibility/)：Player 覆盖 **Il2Cpp 支持的全部目标**（含 WebGL、小游戏、鸿蒙 / 车机等）。
