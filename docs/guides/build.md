...
| 步骤 | 做什么 |
|------|--------|
| 1 | 已 [Install](/docs/guides/install/)（本地 `libil2cpp` / QuickJS / `zents-runtime` 存在） |
| 2 | 菜单 **`ZenTS/Generate/All`**（生成 C++ stub，**不是** C# Wrap） |
| 3 | 纯 JS：将 `JsScripts` 同步到 StreamingAssets；TS：emit 后拷贝 `out/**` → `StreamingAssets/ZenTS/` |
...
确认包内 `ZenTS~/zents-runtime`、`ZenTS~/quickjs-il2cpp` 已由 **`ZenTS/Install...`** 装入 LocalIl2Cpp（输出根一般在 `Library/ZenTS/LocalIl2CppData-…`）。Install 顺序摘要：

1. 同步 Settings `quickjsVersionId` 与 vendored `VERSION`
2. 复制 stock `libil2cpp` 并打 Unity 系列 patch
3. 覆盖拷贝 `zents-runtime` → `libil2cpp/zents`
4. 整目录拷贝 `quickjs-il2cpp` → `libil2cpp/quickjs`
5. 写入 Define / `ZenTSConf.inc`；必要时提示重启 Editor

QuickJS / 原生相关见 [spec/build · QuickJS](/docs/spec/build/01-QUICKJS/)、[多版本管理](/docs/spec/11-MULTI-VERSION/)。

团队内部联调可参考 ZenTSTest：改 `Build-Win64/.../libil2cpp/zents` → 同步回包内 `ZenTS~/zents-runtime`（具体脚本以仓库为准）。

## Generate

- 菜单：**`ZenTS/Generate/All`**
- 依赖：本地 Install 树已存在
- 作用：为 Il2Cpp 生成桥接 stub；改 public API / 换 Unity 后应重跑
- Editor 日常迭代 **不必** 每次 Generate
- TypeScript 声明：**`ZenTS/Generate Typings`**（可挂在 Generate 之后）；类型集须与 Generate **同源**，禁止单独维护另一份 typings 白名单
...
3. 可用 `IPreprocessBuildWithReport` 自动化（参照 [zen-ts-demo](https://github.com/focus-creative-games/zen-ts-demo)）

### TypeScript

1. `tsc --noEmit`（可选但推荐，失败则中断出包）
2. esbuild / `tsc` emit 到 `TsProject/out/`（**禁止** bundle）
3. 拷贝 `out/**/*.js`（及可选 `.js.map`）→ `StreamingAssets/ZenTS/`
...
| 实现 | Expression Emit 等 | C++ stub + `zents-runtime` |
| Generate | 不强制 | **必须** |
