...
---
:::note 文档站副本
本页为语义契约的发布副本；请在上游 `ZTSTest/Docs/spec` 修改后执行 `npm run sync-spec`。（源：`marshal\index.md`）
:::
...
| [02-MARSHAL-AS.md](./02-MARSHAL-AS.md) | `[TsMarshalAs]`、`TsMarshalType`、合法集合、Object/Unpacked/Bytes/Opaque、**XML 外部配置** |
...
| [08-ENUM.md](./08-ENUM.md) | 默认 number + `zts.box` |
...
| `zts.box` / `unbox` / `cast` / `get_opaquevalue` / `set_opaquevalue` | [../05-LIB.md](../05-LIB.md) |
