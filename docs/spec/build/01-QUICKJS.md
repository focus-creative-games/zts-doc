...
---
:::note 文档站副本
本页为语义契约的发布副本；请在上游 `ZenTSTest/Docs/spec` 修改后执行 `npm run sync-spec`。（源：`build\01-QUICKJS.md`）
:::


# 构建 — QuickJS

> 本文约定 ZenTS 在 **Editor（Mono）** 与 **Il2Cpp Player** 上如何接入 **QuickJS**。
> Mono callback gate 见 [03-MONO-CALLBACK-GATE.md](./03-MONO-CALLBACK-GATE.md)。
> 包布局、Install、Define、`ZenTSConf.inc` 见 [11-MULTI-VERSION.md](../11-MULTI-VERSION.md)。
...
ZenTS **仅** 支持 QuickJS 作为 JavaScript 引擎；**不存在** ZLua 式的多引擎矩阵（无 PUC-Rio / LuaJIT 切换）。
...
| Define | `ZENTS_QUICKJS`（及 Install 写入的 Unity 版本相关宏，见 [11-MULTI-VERSION.md](../11-MULTI-VERSION.md) §7） |

**原则：**

- **Editor** 通过 `DllImport("quickjs")` 加载 Plugins 动态库；`ZenTS.Mono` 经 P/Invoke 调用 QuickJS API。
- **Il2Cpp** 中 `zents-runtime` 与 QuickJS 源码 **静态编进同一 native 产物**；不经托管 P/Invoke 热路径。
- Il2Cpp 使用包内 **vendored** 树 **`ZenTS~/quickjs-il2cpp/`**（已含 MSVC/Bee 适配）；**不对 QuickJS 使用 patch 机制**。升版本时直接更新该目录。

---

## 2. Il2Cpp 源码树（权威：`ZenTS~/quickjs-il2cpp`）

上游参考：包外 QuickJS 源码树（开发机本地路径）。包内 **`VERSION`** 记录 pin（当前 **`2026-06-04`** → Settings id `quickjs-2026-06-04`）。
...
| `zents_il2cpp_config.h` / `zents_qjs_std_stubs.c` | MSVC/Il2Cpp 适配与 `js_std_*` stubs |
...
| `zents_qjs_std_stubs.c` | 替代 POSIX `quickjs-libc.c`（Win/Il2Cpp **不**编入 libc.c） |
...
2. **整目录拷贝** `ZenTS~/quickjs-il2cpp/` → **`Local.../libil2cpp/quickjs/`**（去掉 README；确认无 §2.3 文件）。**无** QuickJS patch 步骤。
3. 写入 Scripting Define **`ZENTS_QUICKJS`** 与 **`ZenTSConf.inc`**。
4. 将 **`ZenTS~/zents-runtime`** 覆盖到 **`Local.../libil2cpp/zents`**。

**开发期可编辑参考：** `Build-Win64/Il2CppOutputProject/IL2CPP/libil2cpp/quickjs`；改动回写到 **`ZenTS~/quickjs-il2cpp`**。
...
| **Windows** | `Packages/com.code-philosophy.zen-ts/Plugins/quickjs/win32-x64/quickjs.dll` |
...
| **线程** | ZenTS v1 为 **单主 `JSContext`**（[10-LIFETIME.md](../10-LIFETIME.md) §6）；QuickJS Worker API **不在** v1 规范范围 |
| **bigint** | 引擎支持 bigint，但 ZenTS v1 **禁止** bigint 作为 CLR 整数通道（[00-OVERVIEW.md](../00-OVERVIEW.md) §1.3） |

---

## 6. ES Module 与宿主 loader

ZenTS 宿主在 `JsAppDomain.Initialize` 安装 **ES module loader**（[01-HOST-API.md](../01-HOST-API.md) §1.3），与 QuickJS **`JS_SetModuleLoaderFunc`** / `JS_SetModuleLoaderFunc2` 集成：
...
| **`csharp:` 类型模块** | ZenTS 保留前缀；**不** 经 `moduleLoader`；合成 named export 为 CLR 类型对象（[02-TYPE-SYSTEM.md](../02-TYPE-SYSTEM.md) §2.11） |
...
Install **不** 替换 QuickJS module 系统；ZenTS 在 `zents-runtime` 层挂载 loader opaque 指向托管回调。
...
```
ZenTS~/
├── zents-runtime/              # 安装时 → libil2cpp/zents
│   ├── ZenTSCommon.h
│   ├── QuickJsCompatible.h
│   ├── Il2CppCompatible.h
│   └── generated/
│       └── ZenTSConf.inc       # Install 写入（见 11-MULTI-VERSION §12）
├── patches/
...
**不随包携带** QuickJS 上游完整源码；Install 从 **`Library/ZenTS/QuickJsSrcCache`** 读取（见 [11-MULTI-VERSION.md](../11-MULTI-VERSION.md) §2）。
...
| 文档 | 内容 |
|------|------|
| [11-MULTI-VERSION.md](../11-MULTI-VERSION.md) | UPM、Install 顺序、patch floor、Define、`ZenTSConf.inc` |
